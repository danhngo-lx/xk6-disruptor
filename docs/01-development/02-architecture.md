...
disruptor.injectNetworkFaults({ port: 8080, protocol: "tcp" }, "60s");
...
| `restartTimeout` | `duration` | `"30s"` | Maximum time to wait for container to restart/become ready between kill cycles |

```js
disruptor.injectCrashLoopFault({ container: "my-app", count: 6 }, "300s");

// With custom restart timeout for slower containers
disruptor.injectCrashLoopFault({ container: "my-app", restartTimeout: "60s" }, "300s");
```
...
disruptor.injectHTTPResetPeerFaults({ port: 8080 }, "60s");
...
disruptor.injectNetworkShapingFaults({ delay: 200, jitter: 20, loss: 0.01 }, "60s");

// Rate-limit to 1 Mbit/s
disruptor.injectNetworkShapingFaults({ rate: "1mbit" }, "60s");
```
...
disruptor.injectNetworkPartition({ hosts: ["10.0.1.42"], direction: "both" }, "60s");

// Block outbound traffic to a whole subnet
disruptor.injectNetworkPartition({ hosts: ["10.100.0.0/16"], direction: "egress" }, "60s");
```
...
disruptor.injectCPUStress({ load: 80, cpus: 2 }, "60s");
```
...
disruptor.injectMemoryStress({ bytes: 256 * 1024 * 1024 }, "60s");
```
...
disruptor.injectDiskFill({ bytes: 1 * 1024 * 1024 * 1024, path: "/data" }, "60s");
...
disruptor.injectIOStress({ workers: 4, bytesPerWorker: 10 * 1024 * 1024 }, "60s");

// Stress a specific PVC mount
disruptor.injectIOStress({ path: "/data", workers: 8, bytesPerWorker: 50 * 1024 * 1024 }, "60s");
```
